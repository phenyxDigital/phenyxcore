...
class Connection extends PhenyxObjectModel {
    ...
    public static function setPageConnection(Cookie $cookie, $full = true) {
        ...
        if (!Context::getContext()->phenyxConfig->get('EPH_STATSDATA_PAGESVIEWS') && !Context::getContext()->phenyxConfig->get('EPH_STATSDATA_CUSTOMER_PAGESVIEWS')) {
            return [];
        }
        ...
        if (!Context::getContext()->phenyxConfig->get('EPH_STATSDATA_CUSTOMER_PAGESVIEWS')) {
            return ['id_page' => $idPage];
        }
        $timeStart = date('Y-m-d H:i:s');
        if(isset($cookie->id_connections) && $cookie->id_connections > 0 && $idPage > 0) {
        // The ending time will be updated by an ajax request when the guest will close the page
           
            $values = [$cookie->id_connections, $idPage, '\''.$timeStart.'\''];
    ...
    public static function setNewConnection($cookie) {
        ...
        if (empty($result) && (int) $cookie->id_guest) {
    ...
    public static function cleanConnectionsPages() {
        ...
        $period = Context::getContext()->phenyxConfig->get('EPH_STATS_OLD_CONNECT_AUTO_CLEAN');
